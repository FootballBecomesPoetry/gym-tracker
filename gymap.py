...
GYM_SPLIT = {
    ...
    "Wednesday": {"label": "Active Recovery", "exercises": [
...
def get_daily_row(log_date):
    ...
    cur = conn.execute("SELECT * FROM daily_log WHERE log_date = ?", (log_date,))

    row = cur.fetchone()

    if row is None:

        conn.execute("INSERT INTO daily_log (log_date) VALUES (?)", (log_date,))
...
def save_daily_row(log_date, protein_g, water_l, steps, weight_kg, notes):
    ...
    conn.execute("""
        ...
        UPDATE daily_log SET protein_g=?, water_l=?, steps=?, weight_kg=?, notes=?
...
def get_meal_checks(log_date):
    ...
    cur = conn.execute("SELECT meal, done FROM meal_checks WHERE log_date=?", (log_date,))
...
def set_meal_check(log_date, meal, done):
    ...
    conn.execute("""
        ...
        INSERT INTO meal_checks (log_date, meal, done) VALUES (?, ?, ?)
...
# NEW: meal detail (note + calories + protein) helpers

def get_meal_details(log_date):

    cur = conn.execute(

        "SELECT meal, note, calories, protein_g FROM meal_details WHERE log_date=?", (log_date,)

    )

    existing = {r[0]: {"note": r[1] or "", "calories": r[2] or 0, "protein_g": r[3] or 0} for r in cur.fetchall()}
...
def set_meal_detail(log_date, meal, note, calories, protein_g):
    ...
    conn.execute("""
        ...
        INSERT INTO meal_details (log_date, meal, note, calories, protein_g) VALUES (?, ?, ?, ?, ?)
...
def get_exercise_checks(log_date, exercises):
    ...
    cur = conn.execute("SELECT exercise, done FROM exercise_checks WHERE log_date=?", (log_date,))
...
def set_exercise_check(log_date, exercise, done):
    ...
    conn.execute("""
        ...
        INSERT INTO exercise_checks (log_date, exercise, done) VALUES (?, ?, ?)
...
# NEW: daily calorie/protein totals rolled up from meal_details, for the dashboard
...
def get_meal_macro_totals_for_range(start, end):
    ...
    q = """
        ...
        FROM meal_details WHERE log_date BETWEEN ? AND ? GROUP BY log_date
...
# Date selector (defaults to today, but lets you back-fill)
...
if page == "Today":
    ...
    # Build a full 7-day frame so missing days show as 0
